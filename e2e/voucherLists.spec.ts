...
test.describe('the grouped Transactions screen', () => {
  ...
  test('four headed groups — Sales, Purchase, Inventory, General — each with its voucher types, and their keys', async ({ app }) => {
    await app.getByRole('option', { name: /^Transactions/ }).click();
    await expect(heading(app)).toHaveText('Transactions');
    await expect(app.getByTestId('menu-group').locator('.menu-group-title')).toHaveText(['Sales', 'Purchase', 'Inventory', 'General']);
