...
Deno.serve(
  createIntakeHandler({
    ...
    // a receipt PDF's own text, for the fixed payment-advice rules (Eclipse Combustion): read here, never stored
    pdfText: async (base64: string) => {
      const bytes = Uint8Array.from(atob(base64), (c) => c.charCodeAt(0));
      return (await extractText(await getDocumentProxy(bytes), { mergePages: true })).text;
    },
