-- One thing to create: the Party. A party is a customer, a vendor, or both, and its ledger(s) come with it.
--
--   customer -> a ledger under Sundry Debtors      vendor -> a ledger under Sundry Creditors      both -> one of each
--
-- The ledgers are ordinary ledgers (balance sheet, GST, ageing and credit terms work as they always did); the party is the one
-- thing a person sees. master_apply now commits SEVERAL records in one transaction — the party and its ledgers together, or none.
-- The application decides the ledgers (same domain code as the browser); this file stores them and re-enforces the invariants.
...
alter table public.parties
  ...
  add column shipping jsonb check (shipping is null or jsonb_typeof(shipping) = 'object'),
  add column roles    jsonb check (roles is null or case when jsonb_typeof(roles) = 'array' then jsonb_array_length(roles) > 0 else false end);

alter table public.ledgers
  add column party_role text check (party_role is null or party_role in ('customer', 'vendor')),
  add constraint ledger_party_role_needs_party check (party_role is null or party_id is not null);
create unique index ledgers_party_role_uq on public.ledgers (company_id, party_id, party_role) where party_role is not null;

-- ---------------------------------------------------------------------------------------------
-- Backstops (AFTER the row is written, one small function per table — see the note in the Phase 4 migration)
-- ---------------------------------------------------------------------------------------------
-- A party's ledger sits under the right group and never moves to another party or role.
create function private.protect_party_ledger() returns trigger
language plpgsql
...
declare
  v_want text;
begin
  if tg_op = 'UPDATE' and old.party_role is not null
     and (new.party_id, new.party_role, new.group_id) is distinct from (old.party_id, old.party_role, old.group_id) then
    perform private.raise_issue('UNSUPPORTED_OPERATION', format('"%s" belongs to a party and cannot be moved', old.name));
  end if;
  if new.party_role is not null then
    v_want := case new.party_role when 'customer' then 'sundry-debtors' else 'sundry-creditors' end;
    if not exists (
      with recursive up as (
        select g.id, g.parent_id, g.reserved_key from public.account_groups g where g.id = new.group_id and g.company_id = new.company_id
        union all
        select p.id, p.parent_id, p.reserved_key from public.account_groups p join up on p.id = up.parent_id and p.company_id = new.company_id
      ) select 1 from up where up.reserved_key = v_want
    ) then
      perform private.raise_issue('UNSUPPORTED_OPERATION', format('A %s ledger belongs under %s', new.party_role, case new.party_role when 'customer' then 'Sundry Debtors' else 'Sundry Creditors' end));
    end if;
  end if;
  return null;
end
$$;
create trigger ledgers_party_ledger after insert or update on public.ledgers for each row execute function private.protect_party_ledger();

-- A party can gain a role but never lose one (its ledger may hold entries).
create function private.protect_party_roles() returns trigger
language plpgsql
as $$
begin
  if old.roles is not null and (new.roles is null or not (old.roles <@ new.roles)) then
    perform private.raise_issue('UNSUPPORTED_OPERATION', format('"%s" cannot stop being a customer or vendor once it has been one', old.name));
  end if;
  return null;
end
$$;
create trigger parties_roles after update on public.parties for each row execute function private.protect_party_roles();

-- ---------------------------------------------------------------------------------------------
-- Search: a party shows its roles; its own ledgers are not separate hits
-- ---------------------------------------------------------------------------------------------
create or replace function private.index_master() returns trigger
...
begin
  case tg_table_name
    when 'ledgers' then
      -- A party's own ledgers are reached through the party (one hit per party, as in the application).
      if new.party_role is not null then
        delete from public.search_index where company_id = new.company_id and entity_type = 'ledger' and entity_id = new.id;
        return null;
      end if;
    ...
    when 'parties' then
      ...
      v_sub := concat_ws(' · ',
        (select string_agg(initcap(r), ' / ') from jsonb_array_elements_text(coalesce(nullif(j -> 'roles', 'null'::jsonb), '[]'::jsonb)) r),
        j ->> 'gstin', j ->> 'phone', j ->> 'address');
...
as $$
  select coalesce(jsonb_agg(jsonb_build_object(
           ...
           'code', l.code, 'alias', l.alias, 'party_id', l.party_id, 'party_role', l.party_role, 'reserved_key', l.reserved_key) order by l.name), '[]'::jsonb)
...
-- ---------------------------------------------------------------------------------------------
-- master_apply: commit one validated master change — or several that belong together (a party and its ledgers).
--   p_change = { kind, op, id, row }                       (a single record; still accepted)
--            | { changes: [ { kind, op, id, row }, ... ] }  (applied in order, in this one transaction)
-- The caller has already applied every business rule; this checks the version it validated against, writes the rows in order,
-- bumps the version ONCE and audits each record — all or nothing.
-- ---------------------------------------------------------------------------------------------
