-- Email templates: the subject and body each kind of voucher is emailed with (Sales Invoice, Sales Order, Quotation, Purchase,
-- Purchase Order), keyed by base kind. Null / a kind left out = the built-in default (packages/domain/src/mail/voucherMail.ts).
-- load_masters_json and master_apply are the ones of 20260929000100_invoice_settings with this one column added.

alter table public.companies add column email_templates jsonb;
