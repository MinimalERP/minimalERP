...
import { entryFormFromProposal, inboxBanner, partySeedOf } from '../proposalForms';
...
import { type EntryKind, SALES_KINDS } from '../kinds';
import { useOtherVoucherHandlers } from '../otherVoucher';
import { amountToSettle, billsToOffer, defaultAllocations, settleableBills, unallocated } from '../bills';
import { formatAmount, formatBalance, formatCashBalance, formatDate } from '../format';
 ...
 * VoucherScreen routes here; the item documents and the Stock Journal have their own layouts beside this one.
 */
...
export function LedgerVoucherEntry({ frame, books, mode, typeId, voucher, fromInbox }: LedgerEntryProps) {
  ...
  /** The popup list; the field whose list was closed with Esc. Typing, ↓ or moving to another field opens a list again. */
  const picker = usePickerState(focusKey);
  // a dialog that is asking (party details, "Create what?") has the focus; the field gets it back when it closes
  const { at, current, go, nextKey, prevKey, isFocus } = useFieldFocus({
    fields,
    focusKey,
    setFocusKey,
    rootRef,
  ...
  const { setFieldErrors, issueAt, errorOf, general } = useFieldIssues({ issues: preview.issues, showErrors });
  ...
  useCommandHandler(SCOPE, 'app.back', () => {
    ...
    if (mode !== 'display') {
      ...
      if (previous !== undefined) {
        ...
        go(previous);
        if (previous === 'account' || previous.endsWith('.ledger')) picker.setClosed(previous); // arriving by Esc does not pop the list open
        return true;
      }
    }
    if (dirty) {
  ...
  const save = useVoucherSave({
    ...
    draftKey,
    settle: () => settle(),
    preview: () => previewVoucher(fresh(), layout, masters, kinds),
    ...
    refused: 'That could not be saved',
    cancelRefused: 'That could not be cancelled',
    setShowErrors,
    setFieldErrors,
  ...
  const { acceptAndNew, acceptKey, confirm, setConfirm, banner, setBanner, accept } = save;
  ...
  const pickerList = (key: string) => (
    <PickerList
      ...
      meta={(c) => <span class="row-meta">{books.balanceOf(c.id) !== 0n && <span class="amt">{balanceText(c.id)}</span>}</span>}
    />
  );
