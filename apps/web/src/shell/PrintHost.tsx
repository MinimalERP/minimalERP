  ...
  return (
    <>
      ...
      {print.docs.length > 0 && print.copies && masters && (
        <PrintView docs={print.docs} company={print.docs[0]?.kind === 'report' ? { name: masters.company.name } : printCompanyOf(masters)} copies={print.copies} layouts={books.current?.printLayouts} />
      )}
    </>
  );
}
