...
export function PrintHost() {
  ...
  return (
    <>
      ...
      {print.doc && print.copies && masters && (
        <PrintView doc={print.doc} company={print.doc.kind === 'report' ? { name: masters.company.name } : printCompanyOf(masters)} copies={print.copies} />
      )}
    </>
  );
}
